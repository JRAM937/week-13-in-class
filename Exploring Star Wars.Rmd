...
>Both grab the title of the movie using its index in sw_films. 
...
> Line 59 grabs all values folliwing the pattern "starships."  
Line 60 grabs any value in sw_films whose key is the length of "starships."
Line 61 does something similar to line 60 except it looks for any values within the scope with a length of "starships."
...
sw_ships %>% 
  ...
  ggplot(aes(y = fct_reorder(title, desc(episode)), x=prop)) + 
  #we want bars but our data is already summarized!
  geom_bin_2d(aes(fill = trilogy)) + 
  labs(
...
> There are an overwhelming amount of vehicles with hyperdrive capabilities.
...
sw_peeps %>% 
  ...
  ggplot(aes(x = height, y = mass, color = trilogy))+
